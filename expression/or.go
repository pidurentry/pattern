	...
	tools.ExpressionFactory["or"] = func(expression map[string]interface{}) (tools.Expression, error) {
		...
		for name, value := range expression {
			...
			case "expressions":
				...
				if !ok {
					return nil, errors.New(fmt.Sprintf("'or' expression expects array of objects for 'expressions': %T", value))
				}
			...
			default:
				return nil, errors.New(fmt.Sprintf("unknown key for 'or' expression: %s", name))
			}
		}
		return _or, nil
	}
